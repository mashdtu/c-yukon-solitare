...
/**
 * @file cards.c
 * Implements the functions for managing cards in the game.
 */
...
bool is_valid_card(Card card)
{
    ...
    // Valid suits are HEARTS, DIAMONDS, CLUBS, SPADES (From Suit enum)
...
/**
 * Helper function to find the color of a card based on its suit.
 * Returns true for red cards and false for black cards.
 */
 ...
 * If the current card is a King, returns a "null" card (rank 0, suit 0).
 */
Card get_next_card(Card card)
{
    // Temporary variable to hold the "next" card
    Card next_card;

    // If the rank is less than 13, add 1 to the rank to get the next card
    if (card.rank < 13)
    {
        next_card.rank = card.rank + 1;
        next_card.suit = card.suit;
    }
    // If the rank is 13 there is no next card, so return a "null" card (rank 0, suit 0)
    else
    {
        next_card.rank = 0;
        next_card.suit = 0;
